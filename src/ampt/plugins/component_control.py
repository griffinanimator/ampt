...
class ComponentControl(OpenMayaMPx.MPxLocatorNode):
    ...
    def draw(self, view, path, style, status):
        ...
        def draw_pyramid(offset_vector=None, float_scale=None):

            scale = float_scale or 0.25
            offset = offset_vector or (0, 1, 0)

            glFT.glBegin(OpenMayaRender.MGL_TRIANGLES)
            glFT.glColor3f(0.5, 0.5, 0.5)

            # front face
            glFT.glVertex3f(*offset)
            glFT.glVertex3f(-scale, 0, scale)
            glFT.glVertex3f(scale, 0, scale)
            # left face
            glFT.glVertex3f(*offset)
            glFT.glVertex3f(scale, 0, scale)
